...
void setup(){
  ...
  try{  s = new Server(this, 12345);  }
  catch(Exception e){}
    
  for(int a=0;a<4;a++){
    for(int b=0;b<4;b++){
      S[4*b+a] = new rectbutton(color(200),a*155+5,b*85+5,150,80,4*b+a,"PRESET");  }  }        //upperPage 0
        
  for(int a=0;a<4;a++){
    for(int b=0;b<4;b++){
      S[4*b+a+16] = new rectbutton(color(200),a*155+5,b*85+5,150,80,4*b+a+16,"PRESET");  }  }  //upperPage 1
  ...
  X[0].value = 30;   X[1].value = 50;   X[2].value = 100;   X[3].value = 0;   X[6].value = 0; 
...
void draw(){  //<>//
  fill(0, map((mousePressed && !lockscreen && mouseX<frameSizeX ? 100 : shadeAmount), 0, 100, 255, 10)); noStroke();
  ...
  if(drawTranslateMode  &&  presetPos.size() >= 3){
    ...
    if(translateCounter >= presetPos.size()){  translateCounter = 0;  }
  ...
  if(bpmSTLmode){
    if(millis()-bpmSTLtime >= bpmConstant/bpm){
      ...
      do{  presetColor = color(random(0, 255), random(0, 255), random(0, 255));  }
  ...
  if(millis()-strobeTime >= 200-1.8*presetStrobing){  onoff = !onoff; strobeTime = millis(); }
  if(presetStrobing == 0){  onoff = true; }
    ...
    switch(presetNumber){
      ...
      case 0:  noStroke();
               for(float i=0; i<18; i++){
                 if(multiColor  && i%2 == 0){  fill(presetColor  ,presetBrightness*255/100);  }
                 else if(multiColor)        {  fill(multiColorclr,presetBrightness*255/100);  }
                 ellipse(frameSizeX/2+((0.9*presetSize+10)*height/240)*sin(i*v), height/2+((0.9*presetSize+10)*height/240)*cos(i*v), 0.8*(0.9*presetSize+10), 0.8*(0.9*presetSize+10));  v += float(presetSpeed)/360000;  }
                 break;
      /* 1 Kreis-Ring (Konstant) */
      case 1:  noStroke();
               translate(frameSizeX/2,height/2);
               rotate(v); v += float(presetSpeed)/2000;
               for(float i=0; i<18; i++){
                 if(multiColor  && i%2 == 0){  fill(presetColor  ,presetBrightness*255/100);  }
                 else if(multiColor)        {  fill(multiColorclr,presetBrightness*255/100);  }
      ...
      case 2:  noStroke();
               translate(frameSizeX/2,height/2);
               rotate(v); v += float(presetSpeed)/3000;
               for(float i=0; i<18; i++){
                 if(multiColor  && i%2 == 0){  fill(presetColor  ,presetBrightness*255/100);  }
                 else if(multiColor)        {  fill(multiColorclr,presetBrightness*255/100);  }
               ...
               break;
      /* Random Linien */
      case 3:  strokeWeight(1.4*presetSize+10);
               if(multiColor){
      ...
      case 4:  noFill();
               ...
               if(multiColor){
                 for(float l=0; l<=7; l++){
                   if(l%2 == 0){  stroke(presetColor  ,presetBrightness*255/100);  }
                   else        {  stroke(multiColorclr,presetBrightness*255/100);  }
      ...
      case 5:  noStroke();
               for(float f=0; f<frameSizeX/16; f++){
                 if(multiColor){
                   if(f%20 <= 10){  fill(presetColor  ,presetBrightness*255/100);  }
                   else          {  fill(multiColorclr,presetBrightness*255/100);  }   }
                 ellipse(16*f,height/2+ 4.5*presetSize*sin(v - f/15 /*NumOfPeriods*/) ,70,70);  v += float(presetSpeed)/90000;  }
                 break;
      /* Sinus-Blöcke */
      case 6:  noStroke();
               for(float f=0; f<frameSizeX/90; f++){
                 if(multiColor){
                   if(f%2 == 0){  fill(presetColor  ,presetBrightness*255/100);  }
                   else        {  fill(multiColorclr,presetBrightness*255/100);  }   }
                 rect(100*f, 0.45*height+100*sin(v - f/1)+presetSize*height*0.005, 60, height/10);
                 rect(100*f, 0.45*height-100*sin(v - f/1)-presetSize*height*0.005, 60, height/10);  v += float(presetSpeed)/15000;  }
                 break;
      /* Dreh-Kreuz */
      case 7:  strokeWeight(1.5*presetSize);
               translate(frameSizeX/2,height/2);
               rotate(v); v += float(presetSpeed)/1000;
               if(multiColor){
                 for(float i=0; i<10; i++){
                   if(multiColor  && i%2 == 0){  stroke(presetColor  ,presetBrightness*255/100);  }
                   else if(multiColor)        {  stroke(multiColorclr,presetBrightness*255/100);  }
      ...
      case 8:  noStroke();
               translate(frameSizeX/2, height/2);
               rotate(v);  v += float(presetSpeed)/5000;
               for(int x=0; x<1000; x++){
                 if(multiColor && x>500){  fill(multiColorclr,presetBrightness*255/100);  }
                 ellipse(ranX[x], ranY[x], presetSize/8, presetSize/8);  }
               break;
      /* Random Kreise */
      case 9:  noStroke();
               for(int x=0; x<(presetSize+10)/10; x++){
                 if(multiColor && x>presetSize/20){  fill(multiColorclr,presetBrightness*255/100);  }
      ...
      case 10: noFill();
               ...
               for(int x=0; x<(presetSize+10)/10; x++){
                 if(multiColor && x>presetSize/20){  stroke(multiColorclr,presetBrightness*255/100);  }
               ...
               break;
      /* Scanner seitlich */
      case 11: strokeWeight(1.5*presetSize);
               translate((frameSizeX/2.4)*sin(v)+frameSizeX/2,0); v += float(presetSpeed)/1000;
               if(multiColor){
                 line(0,0,0,height/3);
               ...
               break;
      /* Scanner hoch/runter */
      case 12: strokeWeight(1.5*presetSize);
               translate(0, (height/2.4)*sin(v)+height/2); v += float(presetSpeed)/1000;
               if(multiColor){
               ...
               break;
      /* 2 Scanner */
      case 13: strokeWeight(1.5*presetSize);
               translate((frameSizeX/2.4)*sin(v*1.2)+frameSizeX/2,(height/2.4)*sin(v*0.8)+height/2); v += float(presetSpeed)/1000;
               if(multiColor){
                 for(float i=0; i<10; i++){
                   if(multiColor  && i%2 == 0){  stroke(presetColor  , presetBrightness*255/100);  }
                   else if(multiColor)        {  stroke(multiColorclr, presetBrightness*255/100);  }
      ...
      case 14: noStroke();
               for(int m=0; m<(presetSize+4)/5; m++){
                 if(multiColor && m>(presetSize+4)/10){  fill(multiColorclr, presetBrightness*255/100);  }
                 spotlight[m].move();  }
               break;
      /* Maus-Verfolger-Spot */
      case 15: noStroke();
      ...
      case 16: strokeWeight(presetSize);
               for(int r=-4; r<=4; r++){
                 if(multiColor){
                   if(r%2 == 0){  stroke(presetColor  , presetBrightness*255/100);  }
                   else        {  stroke(multiColorclr, presetBrightness*255/100);  }  }
                 line(m+r*frameSizeX/4,0,m+r*frameSizeX/4,height);  }
               m = m + float(presetSpeed)/3;
               if(m >= frameSizeX-50){  m = -50;  }
               break;
      /* Multiscanner runter */    
      case 17: strokeWeight(presetSize);
               for(int r=-4; r<=4; r++){
                 if(multiColor){
                   if(r%2 == 0){  stroke(presetColor  , presetBrightness*255/100);  }
                   else        {  stroke(multiColorclr, presetBrightness*255/100);  }  }
      ...
      case 18: strokeWeight(80);
               translate(0,(height/3)*sin(v)); v += float(presetSpeed)/5000;
               for(int r=-4; r<=4; r++){
                 if(multiColor){
                   if(r%2 == 0){  stroke(presetColor  ,presetBrightness*255/100);  }
                   else        {  stroke(multiColorclr,presetBrightness*255/100);  }  }
     ...
     /* Sinus-Mini-Multiscanner runter */
      case 19: strokeWeight(80);
               translate((frameSizeX/3)*sin(v),0); v += float(presetSpeed)/5000;
               for(int r=-4; r<=4; r++){
                 if(multiColor){
                   if(r%2 == 0){  stroke(presetColor  ,presetBrightness*255/100);  }
                   else        {  stroke(multiColorclr,presetBrightness*255/100);  }  }
               ...
               break;
      /* Halbkreise */  
      case 20: noFill(); strokeWeight(presetSize);
               if(multiColor && a<14){  stroke(multiColorclr,presetBrightness*255/100);  }
               if     (a<=10 &&  b<=10){  arc(frameSizeX  , height/2, c, c, v*PI/50-HALF_PI, v*PI/50+HALF_PI  , OPEN);  }
               else if(a<=10 &&  b> 10){  arc(frameSizeX/2, height  , c, c, v*PI/50        , v*PI/50+PI       , OPEN);  }
               else if(a> 10 &&  b<=10){  arc(0  , height/2, c, c, v*PI/50+HALF_PI, v*PI/50+3*HALF_PI, OPEN);  }
               else if(a> 10 &&  b> 10){  arc(frameSizeX/2, 0       , c, c, v*PI/50-PI     , v*PI/50          , OPEN);  }
      ...
      case 21: noStroke();
               for(float f=0; f<frameSizeX/16; f++){
                 ...
                 ellipse(16*f,height/2+4.5*presetSize*sin(v - f/15 /*NumOfPeriods*/) ,80,80); v += float(presetSpeed)/90000;  }
                 break;
      /* Rotierende Lines */         
      case 22: strokeWeight(50+0.5*presetSize);
               translate(frameSizeX/2,height/2);
               rotate(v); v += float(presetSpeed)/3000;
               for(int r=-8; r<=8; r++){
                 if(multiColor){
      ...
      case 23: noStroke();
               ...
               break;
      /* 4 Scanner-Linien */
      case 24: strokeWeight(0.8*presetSize);
               ...
               if(multiColor){  stroke(multiColorclr,presetBrightness*255/100);  }
               ...
               break;
      /* 4 rotierende Linien */
      case 25: strokeWeight(0.8*presetSize);
               line(frameSizeX/4+(presetSize*height/240)*sin(500*v),   height/4+(presetSize*height/240)*cos(500*v),   frameSizeX/4-(presetSize*height/240)*sin(500*v),   height/4-(presetSize*height/240)*cos(500*v));
               line(3*frameSizeX/4+(presetSize*height/240)*sin(500*v), 3*height/4+(presetSize*height/240)*cos(500*v), 3*frameSizeX/4-(presetSize*height/240)*sin(500*v), 3*height/4-(presetSize*height/240)*cos(500*v));
               if(multiColor){  stroke(multiColorclr,presetBrightness*255/100);  }
      ...
      case 26: noStroke();
               for(int r=0; r<=3; r++){
                 pushMatrix();
                 switch(r){
                   case 0:  translate(frameSizeX/4,   height/4);   rotate(v);   break;
                   case 1:  translate(3*frameSizeX/4, height/4);   rotate(-v);  break;
                   case 2:  translate(frameSizeX/4,   3*height/4); rotate(-v);  break;
                   case 3:  translate(3*frameSizeX/4, 3*height/4); rotate(v);   break;  }
                   for(float i=0; i<18; i++){
                     if(multiColor  && i%2 == 0){  fill(presetColor  ,presetBrightness*255/100);  }
                     else if(multiColor)        {  fill(multiColorclr,presetBrightness*255/100);  }
      ...
      case 27: noStroke();
               for(int xx=0; xx<=1; xx++){
                 for(int yy=0; yy<=1; yy++){
                   rotate(v); v += float(presetSpeed)/20000;
                   for(float i=0; i<18; i++){
                     if(multiColor  && i%2 == 0){  fill(presetColor  ,presetBrightness*255/100);  }
                     else if(multiColor)        {  fill(multiColorclr,presetBrightness*255/100);  }
      ...
      case 28: noStroke();
               ...
               if(multiColor){  fill(multiColorclr, presetBrightness*255/100);  }
      ...
      case 29: noFill();
               ...
               if(m >= 110){
                 strokeWeight(max(0, (presetSize+1)*(220-m)/110));  stroke(multiColorclr, presetBrightness*255/100);
      ...
      case 30: noStroke();
               for(int i=0; i<=9; i++){
                 for(int j=0; j<=9; j++){
                   if(multiColor){
                     if((i+j)%2 == 0){  fill(presetColor,   constrain(-100+355*sin(v+31*float(i+j)), 0, presetBrightness*255/100));  }
                     else{              fill(multiColorclr, constrain(-100+355*sin(v+31*float(i+j)), 0, presetBrightness*255/100));  }  }
                   else{
  ...
  fill(editFrameSize ? 255 : 0); noStroke();
  ...
  lockscreen = false;
    ...
    if(!pass.equals(realPass)){     // Start (Lock-) screen
      ...
      if(!licenseOK){
        textSize(frameSizeY/30); fill(255, 0, 0);
      ...
      if(!hideControlWindow){
        if(upperPage == 0){
          ...
          for(int i=0; i<=15;  i++){  S[i].display();  }  }
        
        else if(upperPage == 1){
          image(page2presetPix, frameSizeX, 0);
          for(int i=16; i<=31; i++){  S[i].display();  }  }
      
        if(lowerPage == 0){
          for(int i=32; i<=39; i++){  S[i].display();  }
          for(int i=0; i<=3 ;  i++){  X[i].display();  }  }
        
        else if(lowerPage == 1){
          for(int v=0; v<=4; v++){  O[v].display();  }
          for(int v=6; v<=7; v++){  O[v].display();  }
          for(int v=6; v<=6; v++){  X[v].display();  }  }
        
        else if(lowerPage == 2){
          for(int v=5; v<=5; v++){  O[v].display();  }
 ...
 // NETWORKING __________________________________________________________________________________________________________________________________  
    try{  CC = s.available();  }   // Receive data from WiFi Console
    catch(Exception e){  }
    ...
    if(CC != null){
      ...
      for(int k=0; k<vals1.length; k++){
        ...
        //adress 33, 34, 35 now free (formerly Color Buttons Green, Blue, Random)
          ...
          if(_data == 1){  O[_adress-36].buttonState =  true; O[_adress-36].doStuff();  }
          if(_data == 0){  O[_adress-36].buttonState = false; O[_adress-36].doStuff();  }  }
        ...
        // Adress 43 is currently free
          ...
          fromWiFi_X = 0;
          fromWiFi_Y = 0;  }  //Aufteilung von 32 bit auf 2 x 16 bit (0-32768) für X & Y
      }
    }
  }
}
...
void mousePressed(){
    if(mouseButton == LEFT){
      ...
      if(mouseY <= 340){
        upperPage++;
        if(upperPage == 2){  upperPage = 0;  }  }
      
      else{
        lowerPage++;
        if(lowerPage == 3){  lowerPage = 0;  }  }
    }
...
void keyPressed(){
    ...
    if(key == CODED){
      ...
      //}
        ...
        if     (keyCode == LEFT) { frameSizeX = frameSizeX-10; }
        else if(keyCode == RIGHT){ frameSizeX = frameSizeX+10; }
        else if(keyCode == UP)   { frameSizeY = frameSizeY-10; }
        else if(keyCode == DOWN) { frameSizeY = frameSizeY+10; }
        surface.setSize(frameSizeX+cmdWindowWidth, frameSizeY);
      }
    }   
}
...
void drawQuadLED(int _x, int _y, int _bright){
  ...
  if(red(  presetColor) > 0  ||  (multiColor ? red(  multiColorclr) : -1) > 0){ fill(255, 0, 0, _bright*255/100); rect(_x-SIZE-1, _y-SIZE-1, _x-1, _y-1); }
  if(green(presetColor) > 0  ||  (multiColor ? green(multiColorclr) : -1) > 0){ fill(0, 255, 0, _bright*255/100); rect(_x+1, _y-SIZE-1, _x+SIZE+1, _y-1); }
  if(blue( presetColor) > 0  ||  (multiColor ? blue( multiColorclr) : -1) > 0){ fill(0, 0, 255, _bright*255/100); rect(_x-SIZE-1, _y+1, _x-1, _y+SIZE+1); }
  if(red(presetColor) > 0  &&  green(presetColor) > 0  &&  blue(presetColor) > 0){ fill(255, 255, 255, _bright*255/100); rect(_x+1, _y+1, _x+SIZE+1, _y+SIZE+1); }
}
...
class kreis{    // PingPong-Spotlight-Balls
    ...
    void doStuff(){
      if(modifier == "PRESET"){
        ...
        if(index==0){
          ...
          do{  presetColor = color(random(0, 254), random(0, 254), random(0, 254));  }
  ...
  class onoffbutton{
    ...
    void display(){
      ...
      strokeWeight(10);
      rect(xpos+frameSizeX, ypos, xsize, ysize);
      textSize(ysize/6+10); fill(0);
      if(modifier == "BPM"){  text("BPM " + bpm, xmiddle+frameSizeX, ymiddle-4);  }
      else                 {  text(modifier    , xmiddle+frameSizeX, ymiddle-4);  }
    }
    
    void doStuff(){
      if     (modifier == "Multicolor"){  multiColor = buttonState; multiColorclr = presetColor;  }
      else if(modifier == "Line Move" ){  drawTranslateMode = buttonState; fromWiFi_X = 8; fromWiFi_Y = 8;    }  //also reset the extra translation by Smartphone
      else if(modifier == "BPM"       ){  bpmSTLmode = buttonState; bpmSTLtime = 0; bpmSwitchCounter = -1;    }
      else if(modifier == "Blackout"  ){  presetBrightness = buttonState ? 0 : 100;
                                          X[2].value = buttonState ? 0 : 100;          }
      else if(modifier == "+"         ){  bpm = constrain(bpm+1,60,150);               }
      else if(modifier == "-"         ){  bpm = constrain(bpm-1,60,150);               }
      else if(modifier == "Hide (h)"  ){  hideControlWindow = buttonState;             }
      else if(modifier == "Nudge +"   ){  bpmSTLtime -= bpmConstant/(20*bpm);               }
      else if(modifier == "Nudge -"   ){  bpmSTLtime += bpmConstant/(20*bpm);               }
    }
  }
  ...
  class slider{
    ...
    void doStuff(){
      if     (modifier == "Speed"      ){  presetSpeed      = value;  }
      else if(modifier == "Size"       ){  presetSizeDestination       = value;  }
      else if(modifier == "Brightness" ){  presetBrightnessDestination = value;  }
      else if(modifier == "Strobing"   ){  presetStrobing   = value;  }
      else if(modifier == "Shading"    ){  shadeAmount      = value;  }
    }
  }